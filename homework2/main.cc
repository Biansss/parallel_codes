...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < test_number; ++i) {
        ...
        int mode = 2;
        auto res = std::priority_queue<std::pair<float, int>, std::vector<std::pair<float, int>>, std::greater<std::pair<float, int>>>();
        ...
        if(mode == 1) {
            auto res = simflat_search(base, test_query + i*vecdim, base_number, vecdim, k);
        }
        if(mode == 2) {
            auto res = pq_search(base, test_query + i*vecdim, base_number, vecdim, k, 350);
        }
        else{
            auto res = pq_search_simd(base, test_query + i*vecdim, base_number, vecdim, k, 200);
        }
